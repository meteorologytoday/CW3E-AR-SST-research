...
struct BasicMatrixOperators
    ...
    function BasicMatrixOperators(;
       ...
       #println("Making MatrixOperators")
        ...
        U[1:Nx,   :, :] = view(num_U, 2:Nx+1, :, :);    U_W_U = build!(U_I_U_expand, U; wipe=:e)
        U[2:Nx+1, :, :] = view(num_U, 1:Nx,   :, :);    U_E_U = build!(U_I_U_expand, U; wipe=:w)

        V[1:Nx-1, :, :] = view(num_V, 2:Nx, :, :);      V_W_V = build!(V_I_V_expand, V; wipe=:e)
        V[2:Nx,   :, :] = view(num_V, 1:Nx-1, :, :);    V_E_V = build!(V_I_V_expand, V; wipe=:w)
 
        V[:, :, :]      = view(num_UV, 2:Nx+1, :, :);   V_W_UV = build!(UV_I_UV, V)
        V[:, :, :]      = view(num_UV, 1:Nx,   :, :);   V_E_UV = build!(UV_I_UV, V)
